...
namespace robotis_op
{
  ...
  MoveForwartSide::MoveForwartSide()
      : FALL_FORWARD_LIMIT(60),
        ...
        stand_state_(Stand),
        tracking_status_(MoveForwartSideBallTracker::Waiting),
        present_pitch_(0),
        comp_(0),
        komunikasi_moveforwartside_(0),         // komunikasi
        desired_komunikasi_moveforwartside_(0), // komunikasi
        ...
        log_duration_sec_gyro_(61.0), // lama waktu log bno
        start_time_gyro_initialized_(false)
  {
    ...
    boost::thread queue_thread = boost::thread(boost::bind(&MoveForwartSide::callbackThread, this));
    boost::thread process_thread = boost::thread(boost::bind(&MoveForwartSide::processThread, this));
    boost::thread tracking_thread = boost::thread(boost::bind(&MoveForwartSide::trackingThread, this));
  ...
  void MoveForwartSide::setDemoEnable()
  {
    enable_ = true;
    startSoccerMode();
  }

  void MoveForwartSide::setDemoDisable()
  {
    // handle disable procedure
    move_forwart_side_ball_tracker_.stopTracking();
    move_forwart_side_ball_follower_.stopFollowing();
  ...
  void MoveForwartSide::process()
  {
    ...
    {
      move_forwart_side_ball_tracker_.startTracking();
      move_forwart_side_ball_follower_.startFollowing();
      start_following_ = false;
    ...
    if (wait_count_ <= 0)
    {
      ...
      if (on_following_ball_ == true && (komunikasi_moveforwartside_ == 0 || komunikasi_moveforwartside_ == 4 || komunikasi_moveforwartside_ == 10))
      {
        switch (tracking_status_)
        {
        case MoveForwartSideBallTracker::Found:
          move_forwart_side_ball_follower_.processFollowing(move_forwart_side_ball_tracker_.getPanOfBall(), move_forwart_side_ball_tracker_.getTiltOfBall(), 0.0);
          break;

        case MoveForwartSideBallTracker::NotFound:
          move_forwart_side_ball_follower_.waitFollowing();
          break;
      ...
      else if (on_following_ball_ == true && (komunikasi_moveforwartside_ == 42))
      {
        switch (tracking_status_)
        {
        case MoveForwartSideBallTracker::Found:
          move_forwart_side_ball_follower_.waitFollowingLurus();
          break;
      ...
      else if (on_following_ball_ == true && (komunikasi_moveforwartside_ == 43))
      {
        switch (tracking_status_)
        {
        case MoveForwartSideBallTracker::Found:
          move_forwart_side_ball_follower_.waitFollowingMuter();
          break;
      ...
      case Stand:
      {
        ...
        {
          restart_soccer_ = false;
          move_forwart_side_ball_follower_.firstKick();
          startSoccerMode();
          break;
        }

        // check states for kick
        //      int ball_position = move_forwart_side_ball_follower_.getBallPosition();
        bool in_range = move_forwart_side_ball_follower_.isBallInRange();
  ...
  void MoveForwartSide::processThread()
  {
    bool result = false;
    ...
    while (ros::ok())
    {
      if (enable_ == true)
      {
        if ((komunikasi_moveforwartside_ == 0 || komunikasi_moveforwartside_ == 4 || komunikasi_moveforwartside_ == 10 || komunikasi_moveforwartside_ == 42 || komunikasi_moveforwartside_ == 43) && desired_komunikasi_moveforwartside_ != komunikasi_moveforwartside_)
        {
          startSoccerMode();
          desired_komunikasi_moveforwartside_ = komunikasi_moveforwartside_;
        }
        else if (komunikasi_moveforwartside_ != 0 && komunikasi_moveforwartside_ != 4 && komunikasi_moveforwartside_ != 10 && komunikasi_moveforwartside_ != 42 && komunikasi_moveforwartside_ != 43)
        {
          on_following_ball_ = false;
          desired_komunikasi_moveforwartside_ = 5;
          switch (tracking_status_)
          {
          case MoveForwartSideBallTracker::Found:
            move_forwart_side_ball_follower_.processwaitFollowing(move_forwart_side_ball_tracker_.getPanOfBall(), move_forwart_side_ball_tracker_.getTiltOfBall(), 0.0);
            break;
  ...
  void MoveForwartSide::callbackThread()
  {
    ros::NodeHandle nh(ros::this_node::getName());
    ...
    buttuon_sub_ = nh.subscribe("/robotis/open_cr/button", 1, &MoveForwartSide::buttonHandlerCallback, this);
    demo_command_sub_ = nh.subscribe("/robotis/demo_command", 1, &MoveForwartSide::demoCommandCallback, this);
    imu_data_sub_ = nh.subscribe("/robotis/open_cr/imu", 1, &MoveForwartSide::imuDataCallback, this);
    ypr_data_sub_ = nh.subscribe("sensor_ypr", 1, &MoveForwartSide::sensorYPRCallback, this);
    komunikasimoveforwartside_sub_ = nh.subscribe("/receive", 10, &MoveForwartSide::komunikasimoveforwartsideCallback, this); // komunikasi
    rpy_sub_ = nh.subscribe("/bno055/rpy", 10, &MoveForwartSide::rpyCallback, this);
    gyro_sub_ = nh.subscribe("/robotis/raw_gyro", 10, &MoveForwartSide::rawGyroCallback, this); // bno
  ...
  void MoveForwartSide::sensorYPRCallback(const robotis_controller_msgs::SensorYPR::ConstPtr &msg)
  {
    ROS_INFO("Roll : %3.2f, Pitch : %2.2f", msg->yaw, msg->pitch);
  }

  void MoveForwartSide::trackingThread()
  {
    ...
    while (ros::ok())
    {
      if (enable_ == true && on_tracking_ball_ == true)
      {
        ...
        tracking_status = move_forwart_side_ball_tracker_.processTracking();
  ...
  void MoveForwartSide::setBodyModuleToDemo(const std::string &body_module, bool with_head_control)
  {
  ...
  void MoveForwartSide::setModuleToDemo(const std::string &module_name)
  {
    if (enable_ == false)
      return;
  ...
  void MoveForwartSide::callServiceSettingModule(const robotis_controller_msgs::JointCtrlModule &modules)
  {
  ...
  void MoveForwartSide::parseJointNameFromYaml(const std::string &path)
  {
    YAML::Node doc;
    try
    {
  ...
  // joint id -> joint name
  bool MoveForwartSide::getJointNameFromID(const int &id, std::string &joint_name)
  {
  ...
  // joint name -> joint id
  bool MoveForwartSide::getIDFromJointName(const std::string &joint_name, int &id)
  {
  ...
  int MoveForwartSide::getJointCount()
  {
    return joint_id_table_.size();
  }

  bool MoveForwartSide::isHeadJoint(const int &id)
  {
    std::map<std::string, int>::iterator _iter;
  ...
  void MoveForwartSide::komunikasimoveforwartsideCallback(const std_msgs::Int32::ConstPtr &msg) // komunikasi
  {
    ROS_INFO("Balasan Komunikasi Soccerr: %d", msg->data);
    komunikasi_moveforwartside_ = msg->data;
  }

  void MoveForwartSide::buttonHandlerCallback(const std_msgs::String::ConstPtr &msg)
  {
    move_forwart_side_ball_follower_.sendZero();
  ...
  void MoveForwartSide::demoCommandCallback(const std_msgs::String::ConstPtr &msg)
  {
    if (enable_ == false)
      return;
  ...
  std::string MoveForwartSide::waktu()
  {
  ...
  void MoveForwartSide::rawGyroCallback(const geometry_msgs::Vector3Stamped::ConstPtr &msg)
  {
    ...
    system("mkdir -p /home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_balance_side");

    if (!log_initialized_gyro_)
    {
      last_log_time_gyro_ = last_log_time_;
      std::stringstream filename;
      filename << "/home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_balance_side/walking_log_side_" << waktu() << ".txt";
  ...
  void MoveForwartSide::rpyCallback(const geometry_msgs::Vector3Stamped::ConstPtr &msg)
  {
    ...
    system("mkdir -p /home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_bno_side/");

    if (!log_initialized_bno_)
    {
      last_log_time_bno_ = last_log_time_;
      std::stringstream filename;
      filename << "/home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_bno_side/log_side_" << waktu() << ".txt";
  ...
  // check fallen states
  void MoveForwartSide::imuDataCallback(const sensor_msgs::Imu::ConstPtr &msg)
  {
    if (enable_ == false)
      return;
    ...
    system("mkdir -p /home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_open_cr_side/");
    ...
    if (!log_initialized_)
    {
      ...
      std::stringstream filename;
      filename << "/home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_open_cr_side/log_side_" << waktu() << ".txt"; // unik tiap waktu dijalankan
  ...
  void MoveForwartSide::startSoccerMode()
  {
  ...
  void MoveForwartSide::stopSoccerMode()
  {
  ...
  void MoveForwartSide::handleKick(int ball_position)
  {
    ...
    tracking_status_ = MoveForwartSideBallTracker::NotFound;
    move_forwart_side_ball_follower_.clearBallPosition();
  ...
  void MoveForwartSide::handleKick()
  {
    ...
    // kick motion
    move_forwart_side_ball_follower_.decideBallPositin(move_forwart_side_ball_tracker_.getPanOfBall(), move_forwart_side_ball_tracker_.getTiltOfBall());
    int ball_position = move_forwart_side_ball_follower_.getBallPosition();
    if (ball_position == MoveForwartSideBallFollower::NotFound || ball_position == MoveForwartSideBallFollower::OutOfRange)
    {
      on_following_ball_ = false;
      restart_soccer_ = true;
      tracking_status_ = MoveForwartSideBallTracker::NotFound;
  ...
  bool MoveForwartSide::handleFallen(int fallen_status)
  {
    ...
    if (on_following_ball_ == true || komunikasi_moveforwartside_ == 0 || komunikasi_moveforwartside_ == 4 || komunikasi_moveforwartside_ == 1 || komunikasi_moveforwartside_ == 3 || komunikasi_moveforwartside_ == 2)
      restart_soccer_ = true;
  ...
  void MoveForwartSide::playMotion(int motion_index)
  {
  ...
  void MoveForwartSide::setRGBLED(int blue, int green, int red)
  {
  ...
  bool MoveForwartSide::isActionRunning()
  {
  ...
  void MoveForwartSide::sendDebugTopic(const std::string &msgs)
  {
