...
namespace robotis_op
{
  ...
  MoveForwartRotate::MoveForwartRotate()
      : FALL_FORWARD_LIMIT(60),
        ...
        stand_state_(Stand),
        tracking_status_(MoveForwartRotateBallTracker::Waiting),
        present_pitch_(0),
        comp_(0),
        komunikasi_moveforwartrotate_(0),         // komunikasi
        desired_komunikasi_moveforwartrotate_(0), // komunikasi
        log_initialized_(false),
        log_duration_sec_(31.0), // lama waktu log open cr
        log_initialized_bno_(false),
        log_duration_sec_bno_(31.0), // lama waktu log bno
        ...
        start_time_gyro_initialized_(false)
  {
    ...
    boost::thread queue_thread = boost::thread(boost::bind(&MoveForwartRotate::callbackThread, this));
    boost::thread process_thread = boost::thread(boost::bind(&MoveForwartRotate::processThread, this));
    boost::thread tracking_thread = boost::thread(boost::bind(&MoveForwartRotate::trackingThread, this));
  ...
  void MoveForwartRotate::setDemoEnable()
  {
    enable_ = true;
    startSoccerMode();
  }

  void MoveForwartRotate::setDemoDisable()
  {
    // handle disable procedure
    move_forwart_rotate_ball_tracker_.stopTracking();
    move_forwart_rotate_ball_follower_.stopFollowing();
  ...
  void MoveForwartRotate::process()
  {
    if (enable_ == false)
      return;
    ...
    {
      move_forwart_rotate_ball_tracker_.startTracking();
      move_forwart_rotate_ball_follower_.startFollowing();
      start_following_ = false;
    ...
    if (wait_count_ <= 0)
    {
      ...
      if (on_following_ball_ == true && (komunikasi_moveforwartrotate_ == 0 || komunikasi_moveforwartrotate_ == 4 || komunikasi_moveforwartrotate_ == 10))
      {
        switch (tracking_status_)
        {
        case MoveForwartRotateBallTracker::Found:
          move_forwart_rotate_ball_follower_.processFollowing(move_forwart_rotate_ball_tracker_.getPanOfBall(), move_forwart_rotate_ball_tracker_.getTiltOfBall(), 0.0);
          break;

        case MoveForwartRotateBallTracker::NotFound:
          move_forwart_rotate_ball_follower_.waitFollowing();
          break;
      ...
      else if (on_following_ball_ == true && (komunikasi_moveforwartrotate_ == 42))
      {
        switch (tracking_status_)
        {
        case MoveForwartRotateBallTracker::Found:
          move_forwart_rotate_ball_follower_.waitFollowingLurus();
          break;
      ...
      else if (on_following_ball_ == true && (komunikasi_moveforwartrotate_ == 43))
      {
        switch (tracking_status_)
        {
        case MoveForwartRotateBallTracker::Found:
          move_forwart_rotate_ball_follower_.waitFollowingMuter();
          break;
      ...
      case Stand:
      {
        ...
        {
          restart_soccer_ = false;
          move_forwart_rotate_ball_follower_.firstKick();
          startSoccerMode();
          break;
        }

        // check states for kick
        //      int ball_position = move_forwart_rotate_ball_follower_.getBallPosition();
        bool in_range = move_forwart_rotate_ball_follower_.isBallInRange();
  ...
  void MoveForwartRotate::processThread()
  {
    bool result = false;
    ...
    while (ros::ok())
    {
      if (enable_ == true)
      {
        if ((komunikasi_moveforwartrotate_ == 0 || komunikasi_moveforwartrotate_ == 4 || komunikasi_moveforwartrotate_ == 10 || komunikasi_moveforwartrotate_ == 42 || komunikasi_moveforwartrotate_ == 43) && desired_komunikasi_moveforwartrotate_ != komunikasi_moveforwartrotate_)
        {
          startSoccerMode();
          desired_komunikasi_moveforwartrotate_ = komunikasi_moveforwartrotate_;
        }
        else if (komunikasi_moveforwartrotate_ != 0 && komunikasi_moveforwartrotate_ != 4 && komunikasi_moveforwartrotate_ != 10 && komunikasi_moveforwartrotate_ != 42 && komunikasi_moveforwartrotate_ != 43)
        {
          on_following_ball_ = false;
          desired_komunikasi_moveforwartrotate_ = 5;
          switch (tracking_status_)
          {
          case MoveForwartRotateBallTracker::Found:
            move_forwart_rotate_ball_follower_.processwaitFollowing(move_forwart_rotate_ball_tracker_.getPanOfBall(), move_forwart_rotate_ball_tracker_.getTiltOfBall(), 0.0);
            break;
  ...
  void MoveForwartRotate::callbackThread()
  {
    ros::NodeHandle nh(ros::this_node::getName());
    ...
    buttuon_sub_ = nh.subscribe("/robotis/open_cr/button", 1, &MoveForwartRotate::buttonHandlerCallback, this);
    demo_command_sub_ = nh.subscribe("/robotis/demo_command", 1, &MoveForwartRotate::demoCommandCallback, this);
    imu_data_sub_ = nh.subscribe("/robotis/open_cr/imu", 1, &MoveForwartRotate::imuDataCallback, this);
    ypr_data_sub_ = nh.subscribe("sensor_ypr", 1, &MoveForwartRotate::sensorYPRCallback, this);
    komunikasimoveforwartrotate_sub_ = nh.subscribe("/receive", 10, &MoveForwartRotate::komunikasimoveforwartrotateCallback, this); // komunikasi
    rpy_sub_ = nh.subscribe("/bno055/rpy", 10, &MoveForwartRotate::rpyCallback, this);
    gyro_sub_ = nh.subscribe("/robotis/raw_gyro", 10, &MoveForwartRotate::rawGyroCallback, this); // bno
  ...
  void MoveForwartRotate::sensorYPRCallback(const robotis_controller_msgs::SensorYPR::ConstPtr &msg)
  {
    ROS_INFO("Roll : %3.2f, Pitch : %2.2f", msg->yaw, msg->pitch);
  }

  void MoveForwartRotate::trackingThread()
  {
    ...
    while (ros::ok())
    {
      if (enable_ == true && on_tracking_ball_ == true)
      {
        ...
        tracking_status = move_forwart_rotate_ball_tracker_.processTracking();
  ...
  void MoveForwartRotate::setBodyModuleToDemo(const std::string &body_module, bool with_head_control)
  {
  ...
  void MoveForwartRotate::setModuleToDemo(const std::string &module_name)
  {
    if (enable_ == false)
      return;
  ...
  void MoveForwartRotate::callServiceSettingModule(const robotis_controller_msgs::JointCtrlModule &modules)
  {
  ...
  void MoveForwartRotate::parseJointNameFromYaml(const std::string &path)
  {
    YAML::Node doc;
    try
    {
  ...
  // joint id -> joint name
  bool MoveForwartRotate::getJointNameFromID(const int &id, std::string &joint_name)
  {
  ...
  // joint name -> joint id
  bool MoveForwartRotate::getIDFromJointName(const std::string &joint_name, int &id)
  {
  ...
  int MoveForwartRotate::getJointCount()
  {
    return joint_id_table_.size();
  }

  bool MoveForwartRotate::isHeadJoint(const int &id)
  {
    std::map<std::string, int>::iterator _iter;
  ...
  void MoveForwartRotate::komunikasimoveforwartrotateCallback(const std_msgs::Int32::ConstPtr &msg) // komunikasi
  {
    ROS_INFO("Balasan Komunikasi Soccerr: %d", msg->data);
    komunikasi_moveforwartrotate_ = msg->data;
  }

  void MoveForwartRotate::buttonHandlerCallback(const std_msgs::String::ConstPtr &msg)
  {
    move_forwart_rotate_ball_follower_.sendZero();
  ...
  void MoveForwartRotate::demoCommandCallback(const std_msgs::String::ConstPtr &msg)
  {
    if (enable_ == false)
      return;
  ...
  std::string MoveForwartRotate::waktu()
  {
  ...
  void MoveForwartRotate::rawGyroCallback(const geometry_msgs::Vector3Stamped::ConstPtr &msg)
  {
    ...
    system("mkdir -p /home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_balance_rotate");

    if (!log_initialized_gyro_)
    {
      last_log_time_gyro_ = last_log_time_;
      std::stringstream filename;
      filename << "/home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_balance_rotate/walking_log_rotate_" << waktu() << ".txt";
  ...
  void MoveForwartRotate::rpyCallback(const geometry_msgs::Vector3Stamped::ConstPtr &msg)
  {
    ...
    system("mkdir -p /home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_bno_rotate/");

    if (!log_initialized_bno_)
    {
      last_log_time_bno_ = last_log_time_;
      std::stringstream filename;
      filename << "/home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_bno_rotate/log_rotate_" << waktu() << ".txt";
  ...
  // check fallen states
  void MoveForwartRotate::imuDataCallback(const sensor_msgs::Imu::ConstPtr &msg)
  {
    if (enable_ == false)
      return;
    ...
    system("mkdir -p /home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_open_cr_rotate/");
    ...
    if (!log_initialized_)
    {
      ...
      std::stringstream filename;
      filename << "/home/robotis/catkin_ws/src/ROBOTIS-OP3-Demo/op3_demo/src/LOG_ROS/imu_open_cr_rotate/log_rotate_" << waktu() << ".txt"; // unik tiap waktu dijalankan
  ...
  void MoveForwartRotate::startSoccerMode()
  {
  ...
  void MoveForwartRotate::stopSoccerMode()
  {
  ...
  void MoveForwartRotate::handleKick(int ball_position)
  {
    usleep(100);
    ...
    {
    case robotis_op::MoveForwartRotateBallFollower::OnRight:
      std::cout << "Kick Motion [R]: " << ball_position << std::endl;
      playMotion(is_grass_ ? RightKick + ForGrass : RightKick);
      break;

    case robotis_op::MoveForwartRotateBallFollower::OnLeft:
    ...
    tracking_status_ = MoveForwartRotateBallTracker::NotFound;
    move_forwart_rotate_ball_follower_.clearBallPosition();
  ...
  void MoveForwartRotate::handleKick()
  {
    usleep(100);
    ...
    // kick motion
    move_forwart_rotate_ball_follower_.decideBallPositin(move_forwart_rotate_ball_tracker_.getPanOfBall(), move_forwart_rotate_ball_tracker_.getTiltOfBall());
    int ball_position = move_forwart_rotate_ball_follower_.getBallPosition();
    if (ball_position == MoveForwartRotateBallFollower::NotFound || ball_position == MoveForwartRotateBallFollower::OutOfRange)
    {
      on_following_ball_ = false;
      restart_soccer_ = true;
      tracking_status_ = MoveForwartRotateBallTracker::NotFound;
  ...
  bool MoveForwartRotate::handleFallen(int fallen_status)
  {
    ...
    if (on_following_ball_ == true || komunikasi_moveforwartrotate_ == 0 || komunikasi_moveforwartrotate_ == 4 || komunikasi_moveforwartrotate_ == 1 || komunikasi_moveforwartrotate_ == 3 || komunikasi_moveforwartrotate_ == 2)
      restart_soccer_ = true;
  ...
  void MoveForwartRotate::playMotion(int motion_index)
  {
  ...
  void MoveForwartRotate::setRGBLED(int blue, int green, int red)
  {
  ...
  bool MoveForwartRotate::isActionRunning()
  {
  ...
  void MoveForwartRotate::sendDebugTopic(const std::string &msgs)
  {
