    ...
    not_found_count_(0)
{
  ...
  //tambahan
  center_x_sub = nh_.subscribe("center_x", 1, &BallDetector::circlesCallback, this);
...
//tambahan
void BallDetector::circlesCallback(const std_msgs::Float64::ConstPtr& center_x_msg) {
  center_x = center_x_msg->data; // Update center_x from the received message
}
...
void BallDetector::publishCircles()
{
  ...
  //declare the center x for became variabel
  ...
  for (int idx = 0; idx < circles_.size(); idx++)
  {
    ...
    circles_msg_.circles[idx].x += center_x;

    circles_msg_.circles[idx].x = circles_msg_.circles[idx].x<-1?-1:(circles_msg_.circles[idx].x>1?1:circles_msg_.circles[idx].x);
  }
