    ...
    DEBUG(false)
{
  ...
  max_angle_[using_joint_name_["head_tilt"]] = -5 * DEGREE2RADIAN;
...
void HeadControlModule::generateScanTra(const int head_direction)
{
  ...
  case TopLeft:
  {
    ...
    target_position_.coeffRef(0, using_joint_name_["head_tilt"]) = max_angle_[using_joint_name_["head_tilt"]];
    break;
  }
       ...
       state_it != result_.end(); state_it++)
  {
    ...
    // set time
    double calc_moving_time = fabs(goal_position_.coeff(0, index) - target_position_.coeff(0, index)) / (75.0 * M_PI / 180.0);  //kecepatan searching
    if (calc_moving_time > moving_time_)
      moving_time_ = calc_moving_time;
  }
